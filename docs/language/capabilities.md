...
An adapter declaration is resolved independently of workflow allocation. `lab.adapter-bindings.v1` joins its exact Asset IRI to only those owned CapabilityOfferings whose capability-kind and control-mode IRIs the declared driver supports, then records effective activity and separate planning, simulation, and execution eligibility. A Plannable offering does not become Executable because a runtime adapter exists, and an Executable offering does not become operable unless a configured adapter supports its exact control mode.
