...
| Named DNA values referenced by designs | `name: DNA = dna("...")` | references resolve across modules | DNA-typed design property | exact typed reference retained inside generic `design.define` | adapter-dependent |
| `record` plus role membership (`is Event`, `is Evidential`) | yes | yes | yes | yes | no |
| Exact SBOL Component and supplier identities | `sbol_identity`, `supplier_identity` | declarations resolve normally | SBOL identity must be an absolute IRI | separate fields in `lab.portable-module.v13` | active MaterialLot candidates retained in facility evidence and the selected lot frozen in allocated LAIR |
...
| Action ownership and Method refinement contracts | n/a | built-ins plus package-contributed `lab.method-catalog.v2` documents and Python authoring through one validated portable Method registry | ownership modes, exact Intent identities, typed Procedure graphs, absolute capability and property-kind IRIs, and canonical unit IRIs | `refined-alternatives`, `lab.planning-problem.v2`, and `allocated-procedure` | exact facility allocation and adapter invocation |
...
| Dependencies from `Material<Plasmid>` dataflow | yes | resolved generic action operands | yes | Procedure value edges plus exact MaterialLot or Method-output bindings | reviewed facility plans and exact-task adapter documents |
| OT-2 Procedure specialization | n/a | exact allocated task and implementation | canonical Procedure program, exact parameters, units, materials, capability bindings, and profile | `lab.opentrons-ot2-task.v1` plus standalone protocol | generated reviewed protocols only |
...
Python Method authors can use [`lab.procedures.pipetting`](../contributing/python-procedures.md) for typed vessel handles, volume and integer parameter references, transfers, mixes, distributions, and portable techniques. The API serializes existing `PipettingProgramV1` templates; the same Rust validator and refinement path remain authoritative. This authoring API does not expand runtime control flow or the supported physical device features.

Runnable Design/Intent lowering is intentionally narrower than portable-module lowering today: it expands rooted workflow calls and preserves every reachable action in source order, but rejects workflow state, conditionals, matches, loops, triggers, and event emission. These constructs remain fully checked in portable modules; they are never flattened into a misleading linear execution. They will become runnable only when Intent, Method refinement, and planning share explicit control semantics.
