...
| Action ownership and Method refinement contracts | n/a | built-ins plus validated portable Method registry | ownership modes, exact Intent identities, typed Procedure graphs, absolute capability and property-kind IRIs, and canonical unit IRIs | `refined-alternatives`, `lab.planning-problem.v4`, and `allocated-procedure` | exact facility allocation and adapter invocation |
...
| OT-2 Procedure specialization | n/a | exact allocated task operation | exact parameters, units, materials, capability, and profile | `lab.opentrons-ot2-task.v1` plus standalone protocol | generated reviewed protocols only |
