...
Status: provisional, bundled standard-library registry implemented
...
The initial registry resolves `std.bio.parts`, `std.bio.backbones`, `std.bio.inventory`, `std.bio.build`, and `std.lab.plasmid_actions`. It supplies values, pure functions, and action contracts without adding module-specific grammar. This is the interface direction for future packages, not a decision to compile all catalog data permanently into the frontend.
