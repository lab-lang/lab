...
Lab defines versioned, device-neutral operational contracts in `lab_compiler::procedure`. A program-producing Method task either embeds a declarative JSON template or explicitly names a registered builder, and states the exact contract the result must satisfy. Method refinement resolves the task's typed parameters, material inputs, value inputs, and outputs through the template's closed `$lab` slots or gives the same context to the builder before facility planning. The task's descriptive operation IRI is never interpreted as a construction or dispatch key. The resulting `ProcedureProgram` contains observable operations and scientific constraints, but no facility identity, deck coordinate, labware model, pipette model, firmware instruction, endpoint, or credential.
...
1. Method refinement renders the explicit template or invokes the explicit builder, validates its declared contract, stores the complete program on the `procedure.task`, and creates the derived clauses under the Method's execution policy.
...
Matching happens at two levels, because "which Asset can do this work" and "can this code realize this program" are different questions. A `CapabilityFormula` answers the first against SBOLInventory offerings, whose vocabulary describes classes of equipment, and is deliberately coarser than the program. A program feature set and exact profile-feasibility check answer the second against the complete contract value. Features are derived from the canonical program by matching every variant and technique field by name, so extending a contract fails to compile until the new value is mapped, and an implementation that does not declare a feature is refused the program rather than left to render it without. Declaring a feature is a claim that the emitted document preserves the value, not that the planner parses it. The feature check is the outer gate; a concrete adapter still rejects program shapes its own emission cannot render.

Procedure implementation descriptors use stable absolute IRIs and declare the exact Procedure contract version, required fine-grained capability kinds, the program features realized by that implementation, accepted control modes, document formats, and truthful services. Biological operation IRIs and broad adapter capability claims are not compatibility paths. An implementation is eligible only when its contract, features, control mode, capability bindings, and selected operational profile all accept the program.

`PipettingProgramV1` represents logical vessels, task inputs, material sources, products, exact volumes, ordered transfer, distribute, mix, and barrier steps, contamination-path policies, and optional environmental constraints. The explicit Golden Gate setup and serial-dilution builders both emit this contract. OT-2, Flex, and STAR consume the same programs but retain separate resource planners and emit their own reviewed formats.

`ThermalProgramV1` represents the exact task input transformed into an exact output, sample count and fill volume, a global heated-lid setpoint, ordered stages with repeats, exact block plateaus and durations, optional controlled ramp rates, and an optional indefinite final hold. Its contents derive an atomic block-control clause with exact temperature, capacity, and working-volume constraints; an optional heated-lid clause; and an optional controlled-ramp clause. The explicit Golden Gate cycling builder emits this contract. OT-2 and Flex lower it into their distinct Opentrons application formats, while the Inheco ODTC lowers it to the device-neutral reviewed thermal run document already accepted by Lab runtime.
...
- Coarse taxonomy offerings may coexist for discovery, but an implementation of a canonical contract must bind every fine-grained clause the program derives.
