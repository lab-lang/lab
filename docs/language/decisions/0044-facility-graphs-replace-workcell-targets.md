...
The reviewed coordination artifact is `lab.execution-plan.v3`. It freezes inventory, Method, Procedure task, requirement, offering, Asset, MaterialLot, adapter-profile, and reviewed-document hashes in one dependency DAG containing `Execute`, `MoveMaterial`, and `Manual` nodes. One `Execute` node can satisfy a non-empty set of atomic requirements through one reviewed device document, and one `Manual` node can satisfy a non-empty atomic manual requirement set. Device-specific reviewed formats remain independent child documents.
