...
Operational configuration is an overlay keyed by exact Asset IRI. An adapter descriptor states the capability kinds, control modes, document formats, and planning, simulation, or runtime services its implementation supports. Manufacturer and model never select a driver. Endpoints and credentials remain local runtime configuration rather than facility facts.
