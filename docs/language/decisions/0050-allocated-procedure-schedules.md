...
`lab.adapter-invocations.v3` retains each selected Method's explicit completion dependencies, input and output ports, and Procedure yields in addition to its tasks. It carries selected material bindings and the source inventory digest, while the full candidate MaterialLot inventory remains on the facility-planning result. Every adapter-bound task carries a canonical Procedure program and every bound requirement names its exact implementation. A scheduler therefore follows exact selected value edges instead of correlating samples by display names or operation order.
