...
`lab.adapter-invocations.v8` retains each selected Method's explicit completion dependencies, input and output ports, and Procedure yields in addition to its tasks. A scheduler therefore follows exact selected value edges instead of correlating samples by display names or operation order.
