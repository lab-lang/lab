...
The Design dialect represents reusable biological information. One generic `workflow.perform` operation represents every reachable source action with its exact resolved definition, typed arguments and results, ownership, lineage, provenance, and material dataflow without selecting a method or facility resource. The Method dialect represents one or more candidate refinements for an action. Candidate regions contain Procedure tasks and their first-class Capability requirements and must yield compatible result types. The Procedure dialect uses a small closed structural vocabulary with open semantic operation identities, so adding an action, Method, or capability does not require defining a new Pliron operation class.
...
Pliron is the structural substrate of `lab-compiler`: Method and Procedure are parts of the aggregate LAIR domain model rather than a separate representation hidden behind it. Production APIs own `Context`, `ModuleOp`, and `AnalysisManager` together behind stage-typed wrappers, and transformations consume one verified stage to produce the next. `lab-opt` retains a dynamic textual session for inspecting, verifying, and transforming LAIR. Textual LAIR is a compiler-development interface with no external compatibility guarantee.
...
- The solver enumerates the finite facility-feasibility candidates needed to distinguish failure, one complete plan, and ambiguity, and returns decisions that must be applied back to the exact LAIR identities from which they were derived.
- Public Method and adapter extensions use stable owned contracts; they do not require downstream crates or Python packages to depend on Pliron.
