...
Method refinement is facility-independent, but unpinned method selection is not. Refinement enumerates every applicable method candidate and its requirement graph. Planning constructs one constraint problem over method choices, capability offerings, Assets, adapters, MaterialLots, locations, capacity, and scheduling. An explicit source or manifest method pin restricts a method variable to one candidate; otherwise the solver selects a method only when the facility and stated policy make one solution unique. Equally valid solutions remain an explained ambiguity.
