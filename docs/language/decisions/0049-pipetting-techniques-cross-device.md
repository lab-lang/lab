...
Volume tracking is deterministic compiler state. Source fills, dead volume, every aspirate, every dispense, and every mix precondition are validated while planning. Hardware liquid-level detection may verify the planned state but may not replace it. A generated protocol may maintain the same frozen ledger to select already-reviewed positions at runtime when the vendor API requires the final coordinate calculation; the formula and all constants must be embedded in the reviewed child document and covered by adapter tests.
