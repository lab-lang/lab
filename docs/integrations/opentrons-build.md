...
2. A validated Method registry refines every reachable generic Intent operation into candidate Procedure graphs. Program-producing tasks explicitly name the builder and versioned contract that emit their complete canonical Procedure programs, and each program derives its exact fine-grained Capability formula.
3. A read-only LAIR analysis constructs one global planning problem. Facility planning selects Methods, exact active MaterialLots, CapabilityOfferings, Assets, and adapter bindings while preserving the declared dependencies.
...
5. `lab.adapter-invocations.v3` projects immutable selected Methods, exact value edges, and Procedure tasks plus exact Procedure implementation, requirement, offering, Asset, profile, selected material bindings, and inventory and allocated-LAIR digests. Adapter-bound work always carries a canonical Procedure program and exact implementation; programless primitives remain manual and unadapted. The full candidate MaterialLot inventory remains facility-planning evidence instead of being copied into the adapter document. An adapter receives only the tasks and requirements assigned to its exact invocation.
...
The Golden Gate OT-2 vertical slice contains five pipetting tasks and three thermal tasks. The builders explicitly named by `SetupGoldenGateReaction`, `PrepareChemicalTransformation`, `AddRecoveryMedium`, `SerialDilution`, and `PlateDilutedCulture` emit `PipettingProgramV1`. The builders named by `CycleGoldenGateReaction`, `HeatShockTransformation`, and `IncubateRecoveryCulture` emit `ThermalProgramV1`. Transformation preparation and heat shock are separate tasks, as are recovery-medium addition and recovery incubation, so a facility can bind them independently even though the Golden Gate example assigns all eight tasks to one OT-2.

The pipetting programs preserve exact transfers, distributions, mixes, logical vessels, input/output material states, order, volumes, fluid-path policies, and cross-cutting source-temperature constraints. Portable temperature staging, aspiration and dispense strategies, air gaps, blowout, touch-tip, source mixing, final-path reuse, and bubble-clearing movements derive additional atomic capability requirements when present. The bundled `temperature-staged-golden-gate` Method expresses those preparation semantics through `PipettingProgramV1`; it does not change the thermal values authored on the experiment. The thermal programs preserve exact sample count, working volume, ordered stages, repeated plateaus, durations, lid setpoint, optional ramp rates, and final hold. Shared typed Procedure views require the canonical program, its exact derived clauses, parameter types, QUDT units, material roles, and selected material sources before OT-2-specific planning begins. Unsupported contracts, program shapes, missing requirements, and unpreserved values fail explicitly.

The adapter profile realizes portable technique requirements with facility-reviewed calibration. Its resource vocabulary is deliberately independent of Golden Gate: one source area, one thermocycler-backed work area, and small and large tip racks. The example retains the PUDU-derived reduced aspiration rate, 10 mL tracked-source model, 10 mm meniscus offset, 3 mm floor, 2 mm above-liquid dispense, 8 mm material-surface offset, and calibrated touch-tip settings because the canonical pipetting interpreter consumes those values. Assembly, transformation, plating, DNA, dilution, agar, and media do not appear in the profile.
