...
5. `lab.adapter-invocations.v2` projects immutable selected Methods, exact value edges, and Procedure tasks plus exact Procedure implementation, requirement, offering, Asset, profile, selected material bindings, and inventory and allocated-LAIR digests. The full candidate MaterialLot inventory remains facility-planning evidence instead of being copied into the adapter document. An adapter receives only the tasks and requirements assigned to its exact invocation.
