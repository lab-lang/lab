...
The remaining semantic gap is Golden Gate assembly technique. The canonical setup program must explicitly retain PUDU's non-water source mixing, reduced aspiration, blowout, touch-tip, and bottom-to-8-mm bubble-clearing behavior as portable Procedure semantics selected by an optimized Method. The OT-2 renderer may realize those semantics through its calibrated profile, but they must remain visible to facility capability matching and must not become unconditional Python-template behavior for every Golden Gate Method.

The equivalence fixture corpus should include BuildCompiler-derived construct and source-location inputs with expected plasmid, transformation, dilution, and agar mappings checked as semantic data rather than serialized Python. Finally, the official Opentrons simulator should become a CI gate in an environment that pins the supported simulator and Python versions. Flex and STAR must either implement the enriched technique contracts independently or stop advertising those exact Procedure implementations; copying the OT-2 calibration formula would be another abstraction failure.
