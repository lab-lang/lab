...
Transformation preserves 20 µL competent cells, 2 µL DNA, two replicates, multi-plasmid co-transformation, exact DNA-product source wells, source mixing, zero-disposal cell distribution, destination touch-tip, and two bubble-clearing strokes. Heat shock is 4 °C for 1,800 seconds, 42 °C for 60 seconds, and 4 °C for 120 seconds. Recovery adds 60 µL medium with a 10 µL air gap and an above-liquid dispense, then incubates the resulting 82 µL culture at 37 °C for 3,600 seconds.
...
6. assembly liquid and tip actions;
7. transformation liquid and tip actions;
8. plating liquid and tip actions;
...
Liquid traces are reduced only to leaf aspirate, dispense, blowout, touch-tip, pickup, and drop operations. Physical staging wells are compared by the material the generated protocol placed in them, so an arbitrary source-rack packing difference cannot masquerade as a scientific difference. Deck locations that define product lineage, dilution layers, or agar destinations remain exact. The command exits nonzero and retains both normalized documents whenever any required facet differs.

At the pinned revisions, all ten facets pass. The common trace contains 687 equal leaf operations: 220 in assembly, 175 in transformation, and 292 in dilution/plating.
...
- PUDU's generic temperature-module request resolves to GEN1 in the pinned simulator. Lab's facility explicitly declares a GEN2 temperature module. Both outputs use the required Thermocycler Module GEN1.
