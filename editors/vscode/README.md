...
Cursor supports VS Code extensions, so the same package works in both editors. During development, build `lab-language-server`, run `npm install` and `npm run compile` in this directory, then use the editor's Extension Development Host. Set `lab.languageServer.path` when the server is not on `PATH`.
