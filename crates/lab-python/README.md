...
The Python package binds the `lab-lair`, `lab-facility`, and `lab-adapters` domain crates and provides an object model for writing Lab programs in Python. It does not reimplement parsing or semantic checking: the object model emits Lab source and the language's own frontend decides whether it is well formed.
