...
fn require_deck_geometry(profile: &Ot2AdapterProfile) -> Result<(), Ot2PlanningError> {
    ...
    Ok(require_known_geometry(
        "the agar plate",
        stages.plating.agar_plate.capacity,
    )?)
}
...
fn build_assembly_plans(
    ...
    for trace in traces {
        ...
        if end > reaction_plate.len() {
            return Err(
                crate::backend::opentrons::ot2::plan::constraints::plate_capacity_error(
                    "assembly",
                    "reaction_plate",
                    end,
                    reaction_plate.len(),
...
mod tests {
    ...
    use crate::backend::opentrons::ot2::compile_build;
    use crate::backend::opentrons::ot2::plan::build::*;
