...
/// A well on one of the plates a stage may hold several of. `plate` indexes the
/// stage's declared slot list, so adding a slot to a target profile raises the
/// build's capacity without changing any address already assigned.
