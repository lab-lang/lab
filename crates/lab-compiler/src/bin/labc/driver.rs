...
use lab_compiler::{Compiler, compile_module, parse_module, render_checked_module};
...
pub fn run() -> Result<()> {
    ...
    if matches!(
        ...
        let compilation = Compiler
            .compile_backend(&checked, &Ot2Backend)
            .with_context(|| format!("failed to compile OT-2 build {}", cli.source.display()))?;
        let bundle = emit_program(compilation.program()).with_context(|| {
