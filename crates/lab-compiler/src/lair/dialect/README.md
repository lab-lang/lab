...
The structural vocabulary, verifier, portable method contract, built-in vertical-slice registry, and source-to-method refinement path for `refined-alternatives` are implemented. `method.choice` owns one single-block region per candidate Method IRI, `procedure.task` uses open operation and state IRIs with typed SSA ports, `procedure.parameter` preserves exact operation semantics, and `capability.requirement` plus `capability.constraint` preserve qualification, control policy, and exact matching constraints. The global solver extraction, solution application, and `allocated-procedure` stage remain subsequent implementation steps; current `lab build` still follows the transitional fixed Protocol conversion until those downstream stages can replace its v2 checked-AST requirement path.
